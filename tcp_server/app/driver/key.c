...
key_init_single(uint8 gpio_id, uint32 gpio_name, uint8 gpio_func,
		key_function long_press, key_function short_press)
{
	struct single_key_param *single_key = (struct single_key_param *) os_zalloc(
			sizeof(struct single_key_param));
...
key_init(struct keys_param *keys)
{
	...
	for (i = 0; i < keys->key_num; i++)
	{
		...
		PIN_FUNC_SELECT(keys->single_key [ i ]->gpio_name,
				keys->single_key [ i ]->gpio_func);
		...
		GPIO_REG_WRITE(GPIO_STATUS_W1TC_ADDRESS,
				BIT(keys->single_key[i]->gpio_id));

		//enable interrupt
		gpio_pin_intr_state_set(GPIO_ID_PIN(keys->single_key [ i ]->gpio_id),
...
key_50ms_cb(struct single_key_param *single_key)
{
	...
	if (1 == GPIO_INPUT_GET(GPIO_ID_PIN(single_key->gpio_id)))
	{
		...
		gpio_pin_intr_state_set(GPIO_ID_PIN(single_key->gpio_id),
				GPIO_PIN_INTR_NEGEDGE);
	...
	for (i = 0; i < keys->key_num; i++)
	{
		if (gpio_status & BIT(keys->single_key [ i ]->gpio_id))
		{
			...
			gpio_pin_intr_state_set(
					GPIO_ID_PIN(keys->single_key [ i ]->gpio_id),
					GPIO_PIN_INTR_DISABLE);

			//clear interrupt status
			GPIO_REG_WRITE(GPIO_STATUS_W1TC_ADDRESS,
					gpio_status & BIT(keys->single_key[i]->gpio_id));

			if (keys->single_key [ i ]->key_level == 1)
			{
				// 5s, restart & enter softap mode
				os_timer_disarm(&keys->single_key [ i ]->key_5s);
				os_timer_setfn(&keys->single_key [ i ]->key_5s,
						(os_timer_func_t *) key_5s_cb, keys->single_key [ i ]);
			...
			else
			{
				...
				os_timer_setfn(&keys->single_key [ i ]->key_50ms,
						(os_timer_func_t *) key_50ms_cb,
						keys->single_key [ i ]);
