...
LOCAL void uart0_rx_intr_handler(void *para)
{
	...
	fifo_len = (READ_PERI_REG(UART_STATUS(UART0)) >> UART_RXFIFO_CNT_S)
			& UART_RXFIFO_CNT;   //获取buf长度
	uint8 d_tmp = 0;
	uint8 idx = 0, time = 0, send = 0;

	for (idx = 0; idx < fifo_len; idx++)  //原样输出
	{
		d_tmp = READ_PERI_REG(UART_FIFO(UART0)) & 0xFF;
		uart_tx_one_char(UART0, d_tmp);
		read_uart [ idx ] = d_tmp;
		/*time = idx - send * 100;
		 read_uart [ time ] = d_tmp;
		 if (time >= 99)
		 {
		 espconn_send(&esp_conn, read_uart, strlen(read_uart));
		 send++;
		 }*/

		//uart_rev_data[idx]=d_tmp;
	}
...
LOCAL void Uart_Buf_Cpy(struct UartBuffer* pCur, char* pdata , uint16 data_len)
{
	...
	if(tail_len >= data_len)
	{  //do not need to loop back  the queue
